...
const getCollections = async ({ user_id, page, limit }) => {
    ...
    await client.set(cacheCollectionKey, JSON.stringify(responseCollections), "EX", 3600);
...
const deleteCollection = async ({ collection_id, user_id }) => {
    ...
    const keysToDelete = await client.keys(`resource:${user_id}:collection_id:${collection_id}:*`);
...
module.exports = { createCollection, getCollections, deleteCollection, shareCollection, searchCollection };